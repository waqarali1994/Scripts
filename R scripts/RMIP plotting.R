...
# Load data
df <- fread("mlm_yield_results_earLength.y.csv.gz")
df$support <- log10(df$earLength.y.MLM)*-1
df$support
...
p <- ggplot() +
  ...
  scale_y_continuous(expand = c(0, 0.02), limits = c(-.02, 10)) +
  common_theme
...
#for (i in seq_along(formatted_trait_names)) {
 ...
 # }
#}
df_plot <- gwas_data[gwas_data$support>6]  
p <- p + geom_point(data = df_plot,
                                            aes(x = bp_cum, y = support),
                       alpha = 0.9, size = 4) 
